...
def markdown_skill_refs(path: Path) -> set[str]:
    return set(re.findall(r"`(kujo-[a-z0-9-]+)`", path.read_text(encoding="utf-8")))


def validate_package(skill_names: set[str]) -> None:
    index_names = set(re.findall(r"^\| `(kujo-[a-z0-9-]+)` \|", (REPO_ROOT / "SKILLS_INDEX.md").read_text(encoding="utf-8"), re.MULTILINE))
